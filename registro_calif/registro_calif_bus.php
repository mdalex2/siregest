...
<form id="form_buscar" name="form_buscar" action="<?php echo "registro_calif.php?id_func=".$_GET["id_func"]."&accion=administrar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
<tr>
  ...
  <label for="cmb_asig">Asignatura / programa</label>
  <input type="hidden" name="txt_nom_asig" id="txt_nom_asig" value="">
  <br>
  <SELECT name="cmb_asig" id="cmb_asig" SIZE=1 class="lf validate[required]" style="width:95%" onChange="$('#txt_nom_asig').val($('#cmb_asig option:selected').html());"> 
  <option value="">SELECCIONE...</option>

  </SELECT>
  <p class="formHint"> (*) asignatura pendiente</p>
</div></td>
  <td>
    <button formaction="<?php echo "registro_calif.php?id_func=".$_GET["id_func"]."&accion=administrar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit"> <span class="ui-button-text"><img src="../images/icons_menu/x32/find_x32.png" width="20" height="20" align="absmiddle">&nbsp;Mostrar estudiantes</span> </button></td>
</tr>
</table>   
</fieldset>            
</form>
