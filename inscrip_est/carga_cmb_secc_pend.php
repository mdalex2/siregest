<?php
  ...
  $cod_grado= $_REQUEST["id"];
	...
	) where inst_secciones.cod_plantel='$cod_plantel' AND inst_secciones.cod_grado='$cod_grado' AND (inst_secciones.seccion_corto='U' or inst_secciones.seccion_corto='*') AND inst_secciones.visible=true order by inst_secciones.seccion_corto ASC",false);
	//si hay registros para mostrar
  if (!$consulta==false){
	while ($fila=mysql_fetch_array($consulta)){
		echo utf8_encode("<OPTION VALUE='".$fila['id_seccion']."'>".$fila["seccion_corto"]." | ".$fila["mencion"]."</OPTION>");
	}
	}
