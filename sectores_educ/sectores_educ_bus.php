...
<form id="form_buscar" name="form_buscar" action="<?php echo "sectores_educ.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="sector_educ" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="sector_educ") echo " selected " ?>>SECTOR EDUCATIVO</OPTION>
<OPTION VALUE="id_sector_educ" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="id_sector_educ") echo " selected " ?>>C&Oacute;DIGO DEL SECTOR</OPTION>
</SELECT>
...
<div class="ctrlHolder">
  ...
  <label for="txt_buscar" id="lbl_text_busc"><?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba sector educativo a buscar"; ?></label><br>
...
<td>  
<button formaction="<?php echo "sectores_educ.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select id_sector_educ,sector_educ,visible,sectores_educ.fecha_g,datos_per.nombres,datos_per.apellidos from (sectores_educ INNER JOIN datos_per on sectores_educ.guardado_por=datos_per.id_personal) ORDER BY sector_educ ASC",true);} else {
		$consulta=ejecuta_sql("select id_sector_educ,sector_educ,visible,sectores_educ.fecha_g,datos_per.nombres,datos_per.apellidos from (sectores_educ INNER JOIN datos_per on sectores_educ.guardado_por=datos_per.id_personal) where  $campo_buscar LIKE '%$texto_buscar%' ORDER BY sector_educ ASC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="sectores_educ.php?id_func=".$_GET['id_func']."&accion=mostrar&id_mos=".$fila['id_sector_educ'];
					echo '
...
<table id="tablasinbtn" class="letra_16 mouse_hover" width="..."> 
        ...
        <?php
				...
				?>
          <tr>
            <td align="center"><?php echo $fila["id_sector_educ"]?></td>
            <td><?php echo $fila["sector_educ"]?></td>
            <td align="center"><?php 
            ...
            <td align="center">
 							...
 							<a id="resize" href="sectores_educ.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_mos={$fila['id_sector_educ']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
