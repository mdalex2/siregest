...
$(document).ready(function() {
  ...
  <body background="">
<?php
if (isset($_GET["accion_dir"])){
	$accion=strtoupper($_GET["accion_dir"]);
	switch ($accion){
		case "PASAR_DIR":
...
?>
<form id="form" name="form"  action="inst_educativa_agr_dir.php?id_func=<?php if (isset($_GET["id_func"])){echo $_GET['id_func'];}?>&accion_dir=pasar_dir&accion=<?php if (isset($_GET["accion"])){echo $_GET['accion'];}?>&id_mos=<?php if (isset($_GET["id_mos"])){echo $_GET['id_mos'];}?>"  method="post" enctype="multipart/form-data" class="uniForm">
										<!-- Fieldset -->
	<fieldset>
										  <!--<legend>Configuración:</legend>-->
                      <h2 align="left" class="titulo_form_blue">
                      <img src="../images/world_add.png" width="32" height="32" align="absmiddle"> SELECCIONAR UBICACI&Oacute;N DEL PLANTEL</h2>
...
?>
              ...
              <br>
              <SELECT NAME="cmb_mcpio" id="cmb_mcpio" SIZE=1  class="mf validate[required]" onchange = "$('#cmb_par option:selected').removeAttr('selected');$('#cmb_pob option:selected').removeAttr('selected');this.form.submit()"> 
                <option value="">SELECCIONE...</option>
                <?php 
if ($consulta && mysql_num_rows($consulta)>0){
while ($fila=mysql_fetch_array($consulta)) {
...
?>
              ...
              <br>
              <SELECT NAME="cmb_par" id="cmb_par" SIZE=1  class="mf validate[required]" onchange = "$('#cmb_pob option:selected').removeAttr('selected');this.form.submit()"> 
                <option value="">SELECCIONE...</option>
                <?php 
if ($consulta && mysql_num_rows($consulta)>0){
while ($fila=mysql_fetch_array($consulta)) {
...
<tr><td >
					<button formaction="inst_educativa_agr_dir.php?id_func=<?php if (isset($_GET["id_func"])){echo $_GET['id_func'];}?>&accion_dir=pasar_dir&accion=<?php if (isset($_GET["accion"])){echo $_GET['accion'];}?>&id_mos=<?php if (isset($_GET["id_mos"])){echo $_GET['id_mos'];}?>"  class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit" >
<span class="ui-button-text"><img src="../images/sistema/select.png" width="24" height="24" align="absmiddle">&nbsp;Seleccionar</span>
</button>
