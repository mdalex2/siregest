...
{
var agree=confirm("¿Está seguro que desea aplicar la configuración actual al plan de estudio seleccionado? \n\n Esto provocará que los reportes de notas y afínes se muestren con la nueva configuración, favor seleccionar todas las materias antes de guardar la configuración; tambien quite filtro de búsqueda de materias o asignaturas si tiene aplicado alguno");
if (agree)
return true ;
else
return false ;
...
return false ;
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=<?php if (isset($_SESSION['juego_caracteres'])){ echo $_SESSION['juego_caracteres'];} else echo "utf-8";//esta variable se declara en modulo verificar login?>">
	<title><?php echo $_SESSION["app_nombre"]."-".$_SESSION["app_version"]; ?> - Administrar plan de estudios</title>
  <!-- End of Meta -->
    <!-- Scripts Graybox -->
  ...
  <!-- para las validaciones del formulario es -->  
	...
	<script>
		jQuery(document).ready( function() {
			...
			jQuery("#form_plan_est_adm").validationEngine({autoHidePrompt:true,autoHideDelay: 3500});									
...
$(document).ready(function() {
 ...
 -->
					...
					<h1 align="left" class="titulo_form_blue">
                      ...
                      " width="32" height="32" align="absmiddle"> M&Oacute;DULO: PLAN DE ESTUDIO 
<?php 
if (isset($_GET["accion"])){
  $accion=strtoupper($_GET["accion"]);
...
?>
 ...
 <?php
					 if (!isset($_GET["accion"])){
						 ...
						 switch ($accion){
							 case "BUSCAR":
							 	include_once("plan_est_bus.php");
								break;
							 case "NUEVO":
							 	include_once("plan_est_crea.php");
								break;
							 case "MOSTRAR":
						    if (isset($_SESSION["msg"]) && $_SESSION["msg"]!=""){
							 	...
							 	include_once("plan_est_mos.php");
								break;	
								case "GUARDAR":
							 	include_once("plan_est_guardar.php");
								guardar_datos(); //llamo a la funcion que me guarda los datos en la BD
								break;			
								case "MODIFICAR":
							 	include_once("plan_est_modif.php");
								modif_datos(); //llamo a la funcion que me guarda los datos en la BD
								break;
								case "ELIMINAR":
									include_once("plan_est_elim.php");
									eliminar_datos();
									break;																			
								case "ADMINISTRAR":
									include_once("plan_est_admin.php");
									break;	
								case "GUARDAR_ASIG":
									include_once("plan_est_gua_asi.php");
									break;																			
								case "IMPRIMIR_ASIGNACION":
          ...
          <a id="nb" href="<?php echo "plan_est.php?id_func=".$_GET["id_func"]."&accion=buscar"?>"  title="Consultar datos">
                                <img src='../images/icons_menu/x32/search_x32.png' width='24px' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Buscar plan estudio
                              </a><br> 
            <?php 
            ...
            <?php          
						if ($array_permisos["crear"]==true) {?>
								<a id="an" href="<?php echo "plan_est.php?id_func=".$_GET["id_func"]."&accion=nuevo" ?>"  title="Agregar nuevo">
									<img src='../images/sistema/computer_add.png' width='24px' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Crear plan estudio&nbsp;               
                  </a><br>              
          ...
          <?php
						...
						if ($array_permisos["crear"]==true) {?>
								<a id="an" href="<?php echo "plan_est.php?id_func=".$_GET["id_func"]."&accion=administrar" ?>"  title="Permite agregar o quitar las asignaturas y programas para el plan de estudio seleccionado">
									<img src='../images/sistema/computer_go.png' width='24px' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Administrar plan de estudio&nbsp;               
                  </a><br>              
          <?php } //cierro el si tiene permiso de crear ?>
        <?php 
				if ($array_permisos["mostrar"]==true) {?>
          <a id="nb" href="<?php echo "plan_est.php?id_func=".$_GET["id_func"]."&accion=imprimir_asignacion"?>"  title="Ver asignación de materias al plan de estudio">
