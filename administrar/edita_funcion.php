...
<style>
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<title>SIREGEST V.2012.1 - Módulo editar funciones</title>
  <!-- End of Meta -->
    <!-- Scripts Graybox -->
...
<?php
	...
	//si no existe el directorio predeterminado lo creo
	if (!file_exists($_SESSION['ruta_iconos32'])){
		rmkdir($_SESSION['ruta_iconos32']);}
	$id_ant=$_POST["txt_id_ant"]; //id anterior oculto en el form editar
	$archivo32=$_POST["txt_icono_anterior"];
	$archivo48=$_POST["txt_icono_anterior"];
	$msg_icono="";
	$id_func=eliminar_comillas($_POST["txt_id_func"]);
	$texto_icono=eliminar_comillas($_POST["txt_texto_icono"]);
	$descrip_func=eliminar_comillas($_POST["txt_descripcion_func"]);
	$url=eliminar_comillas($_POST["txt_texto_enlace"]);
	$id_padre=eliminar_comillas($_POST["txt_padre"]);
	$icono=eliminar_comillas($_FILES["txt_icono"]["name"]);
	if (isset($_POST["chk_raiz"]) and $_POST["chk_raiz"]=="on" ){
		$es_raiz=true;} 
	else 
		{$es_raiz=false;}
		
	if (isset($_POST["chk_func_act"]) and $_POST["chk_func_act"]==true ){
		$activa='true';} 
	else 
		{$activa='false';}		
	...
	if(!empty($_FILES['txt_icono']) && $_FILES['txt_icono']['error'] == UPLOAD_ERR_OK) {
			...
			if (!file_exists($_SESSION['ruta_iconos48'])){
		rmkdir($_SESSION['ruta_iconos48']);}
			...
			{
				$msg_icono=", sin embargo el ícono no se modificó porque usted no selecciono una imagen. Si usted seleccionó una imagen es probable que la imagen sea inválida o tenga un formato incorrecto";
	...
	icono: $icono <hr>
	activa: $activa";*/
			$sql="update sis_funciones set
			id_func='$id_func',
			id_padre='$id_padre',
			texto_icono='$texto_icono',
			icono32='$archivo32',
			icono48='$archivo48',
			descrip_func='$descrip_func',
			orden=$orden,
			url='$url',
			activa=$activa 
		WHERE id_func ='$id_ant'";
			$conexion=conectarse();
			$consulta=mysql_query($sql,$conexion);
			if ($consulta){
				unset($_SESSION['var_menu']);
				echo "<br>".mostrar_box("inf",false,"Registro modíficado","El registro se modificó correctamente en la base de datos".$msg_icono);
			} else {
				switch (mysql_errno()){
				case 1062: 
