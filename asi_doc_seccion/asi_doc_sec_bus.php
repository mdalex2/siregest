...
<form id="form_buscar" name="form_buscar" action="<?php echo "asi_doc_sec.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
<td width="69%">
  <div class="ctrlHolder">
    ...
    <input type="hidden" id="lbl_text_busc1" name="lbl_text_busc1" value="<?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba la secci&oacute;n a buscar"; ?>">
     <button formaction="<?php echo "asi_doc_sec.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (!empty($_POST["txt_bus"]) && !empty($_POST["cmb_plan"]) && isset($_GET['id_func'])){
		...
		if (empty($_POST["cmb_gra"])){
			$filtro_grado="";
				...
				$filtro_grado=" AND inst_secciones.cod_grado='".$_POST["cmb_gra"]."'";
			}
		...
		) where inst_secciones.cod_plantel='$cod_plantel' AND inst_secciones.visible=true ".$filtro_grado." ORDER BY grados_esc.orden",true);}
		else
		{
		$consulta=ejecuta_sql("select id_seccion,seccion_largo,seccion_corto,inst_secciones.fecha_g,grados_esc.grado_letras,inst_secciones.visible,datos_per.nombres,datos_per.apellidos,menc_edu.mencion,plan_est_tip.nivel_plan_est,sectores_educ.sector_educ from (inst_secciones 
		INNER JOIN datos_per on inst_secciones.guardado_por=datos_per.id_personal 
		...
		INNER JOIN plan_est_tip ON plan_est_tip.id_plan_nivel_est=inst_secciones.id_plan_nivel_est
		INNER JOIN sectores_educ ON sectores_educ.id_sector_educ=inst_secciones.id_sector_educ
		) where inst_secciones.cod_plantel='$cod_plantel' and seccion_corto like '%".$texto_buscar."%' AND  inst_secciones.visible=true ".$filtro_grado,true);
		}
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="asi_doc_sec.php?id_func=".$_GET['id_func']."&accion=administrar&id_mos=".$fila['id_seccion'];
					echo '
...
<table id="tablasinbtn" class="letra_16 mouse_hover" > 
        <thead> 
          <tr>
            <th width="85px">MENCI&Oacute;N</th>
        ...
        <?php
				...
				?>
          <tr>
            <td align="center"><?php echo $fila["mencion"]?></td>
            ...
            <td align="center">
 							...
 							<a id="resize" href="asi_doc_sec.php?<?php echo "id_func=".$_GET['id_func']."&accion=administrar&id_mos={$fila['id_seccion']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Administrar &eacute;sta secci&oacute;n" >&nbsp;
                  <img src='../images/sistema/computer_edit.png' width='18' height="18" heigth='16px' align='absmiddle'></img>&nbsp;Admin
              </a>            </td>
            <?php 
              }
            ?>
