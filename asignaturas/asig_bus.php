...
<form id="form_buscar" name="form_buscar" action="<?php echo "asignaturas.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="AS" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="AS") echo " selected " ?>>ASIGNATURA</OPTION>
<OPTION VALUE="PR" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="PR") echo " selected " ?>>PROGRAMA</OPTION>
<OPTION VALUE="OT" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="OT") echo " selected " ?>>OTRO</OPTION>
</SELECT>
...
<div class="ctrlHolder">
  ...
  <label for="txt_buscar" id="lbl_text_busc"><?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba el nombre de la asignatura"; ?></label><br>
...
<td>  
<button formaction="<?php echo "asignaturas.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select cod_asig_prog,des_mat_prog,visible,asig_prog.fecha_g,datos_per.nombres,datos_per.apellidos from (asig_prog INNER JOIN datos_per on asig_prog.guardado_por=datos_per.id_personal) ORDER BY des_mat_prog,orden ASC",true);} else {
		$consulta=ejecuta_sql("select cod_asig_prog,des_mat_prog,visible,asig_prog.fecha_g,datos_per.nombres,datos_per.apellidos from (asig_prog INNER JOIN datos_per on asig_prog.guardado_por=datos_per.id_personal) where tip_asig='$campo_buscar' AND des_mat_prog like '%".$texto_buscar."%' ORDER BY des_mat_prog,orden ASC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="asignaturas.php?id_func=".$_GET['id_func']."&accion=mostrar&id_mos=".$fila['cod_asig_prog'];
					echo '
...
<table id="tablasinbtn" class="letra_16 mouse_hover" width="..."> 
        <thead> 
          <tr>
            <th width="85px">COD. ASIG    </th>
            <th width="...">ASIGNATURA O PROGRAMA             </th>
        ...
        <?php
          ...
          <tr>
            ...
            <td align="center">
 							...
 							<a id="resize" href="asignaturas.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_mos={$fila['cod_asig_prog']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
