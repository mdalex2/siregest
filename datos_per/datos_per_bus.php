...
<form id="form_buscar" name="form_buscar" action="<?php echo "datos_per.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
<td>  
<button formaction="<?php echo "datos_per.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_cedula"]) && isset($_POST["cmb_tip_doc_b"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select id_personal,num_identificacion,nombres,apellidos,sis_grupos.nombre_grupo_usuario from (datos_per INNER JOIN sis_grupos ON sis_grupos.id_grupo=datos_per.id_grupo) where id_personal<>'CIS_ADMIN'",true);}  else {
			
		$consulta=ejecuta_sql("select id_personal,num_identificacion,nombres,apellidos,sis_grupos.nombre_grupo_usuario from (datos_per INNER JOIN sis_grupos ON sis_grupos.id_grupo=datos_per.id_grupo) where $campo_buscar like '%".$texto_buscar."%' AND id_personal<>'CIS_ADMIN' ",true);} 
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="datos_per.php?id_func=".$_GET['id_func']."&accion=mostrar&id_per=".$fila['id_personal'];
					echo '
				...
				{
?>
<table id="tablasinbtn" class="letra_16 mouse_hover" width="659"> 
        <thead> 
          <tr>
            <th width="85px">N&deg; CEDULA    </th>
            <th width="190">NOMBRES             </th>
            <th width="190">APELLIDOS           </th>
            <th title="TIPO DE EXPEDIENTE" width="80px">TIPO</th>
            <?php if ($array_permisos["mostrar"]==true) {?>
            <th title="ABRIR EXPEDIENTE" width="80px">ABRIR.</th>
            <?php }?>
          </tr> 
        </thead> 
        <tbody> 
        <?php
          ...
          <tr>
            ...
            <td align="left"><?PHP echo $fila["nombre_grupo_usuario"]?></td>
            ...
            <td align="center">
 							...
 							<a id="resize" href="datos_per.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_per={$fila['id_personal']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Abrir expediente personal" >&nbsp;
