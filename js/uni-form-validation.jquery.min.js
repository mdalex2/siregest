...
$message=$('<div />').attr('id','okMsg').html("<h3>"+title+"</h3>");form.prepend($message);$('html, body').animate({scrollTop:form.offset().top},500);$('#okMsg').slideDown();return false;};jQuery.fn.uniform.language= {   required      : '%s Requerido',
    req_radio     : 'Debe seleccionar una opción',
    req_checkbox  : 'Debe seleccionar el checkbox para continuar',
    minlength     : '%s should be at least %d characters long',
    min           : '%s debe ser mayor o igual que %d',
    maxlength     : '%s should not be longer than %d characters',
    max           : '%s debe ser menor o igual que %d',
    same_as       : 'La %s es diferente de %s',
    email         : '%s no es v&aacute;lido',
    url           : '%s no es una web válida',
    number        : '%s se requiere un número',
    integer       : '%s debe contener sólo números enteros',
    alpha         : '%s Debe escribir sólo letras (sin carácteres especiales o números)',
    alphanum      : '%s Debe escribir solo letras y n&uacute;meros (sin caracteres especiales como: *()& ...)',
    phrase        : '%s puede contener cualquier tipo de texto',
    phone         : '%s debe escribir un número válido en formato: (0242)-111-1111',
    date          : '%s debe escribir un fecha corta ejm (mm/dd/yyyy mm=mes; dd= dia; yyyy= año)',
    ...
    submit_msg    : '<img src="../images/icns_mensajeria/excl_16-1.png" align="absmiddle"></img> Atención, debe corregir los siguientes problemas:',
    submit_help   : 'Favor verifíque los campos resaltados.',
    submit_success: 'Los datos se han enviado.'
};
;jQuery.fn.uniform.defaults={submit_callback:false,prevent_submit:false,prevent_submit_callback:false,ask_on_leave:false,on_leave_callback:false,valid_class:'valid',invalid_class:'invalid',error_class:'error',focused_class:'focused',holder_class:'ctrlHolder',field_selector:'input, textarea, select',default_value_color:"#AFAFAF"};