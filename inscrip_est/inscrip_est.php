...
{
var agree=confirm("¿Está seguro de eliminar este usuario?");
if (agree)
return true ;
else
return false ;
}
function confirm_insc()
{
var agree=confirm("Está a punto de generar una inscripción\n\n¿Está seguro que verificó la información y desea procesar la inscripción?");
if (agree)
return true ;
else
return false ;
...
return false ;
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=<?php if (isset($_SESSION['juego_caracteres'])){ echo $_SESSION['juego_caracteres'];} else echo "utf-8";//esta variable se declara en modulo verificar login?>">
	<title><?php echo $_SESSION["app_nombre"]."-".$_SESSION["app_version"]; ?> - Inscripci&oacute;n de estudiantes</title>
  <!-- End of Meta -->
    <!-- Scripts Graybox -->
...
$(document).ready(function() {
		...
		$("#cmb_anno_esc").change(function(){
			...
			var cod_plantel = $('#cmb_pla option:selected').val();
			
		$.post("carga_cmb_grado.php?cod_plantel="+cod_plantel,{ id:$(this).val() },function(data){$("#cmb_gra").html(data);})
		
	});//------- FIN CARGA COMBO GRADO	
	
		//CARGO LISTA DE SECCIONES PARA MATERIA PENDIENTE
		$("#cmb_gra_pen").change(function(){
			//obtengo el codigo del plantel
			var cod_plantel = $('#txt_cod_plant_ocu').val();
			
		$.post("carga_cmb_secc_pend.php?cod_plantel="+cod_plantel,{ id:$(this).val() },function(data){$("#cmb_secc_pend").html(data);})
		
	});//------- FIN CARGA COMBO SECCIONES PARA MATERIA PENDIENTES	
	// --- LLENAR COMBO SECCION
		...
		// --- LLENAR COMBO ASIGNATURA PENDIENTE
		$("#cmb_secc_pend").change(function(){
			//obtengo el codigo del plantel
			//var cod_seccion = $('#txt_secc_ocu').val();
			
		//$.post("carga_cmb_asig_pend.php",{ id:$(this).val() },function(data){$("#cmb_asi_pen").html(data);})
		$.post("carga_chk_asig_pend.php",{ id:$(this).val() },function(data){$("#div_asig_pend").html(data);})
		//------- FIN CARGA COMBO ASIGNATURAS PENDIENTES
	});	
 ...
 -->
					...
					<h1 align="left" class="titulo_form_blue">
                      ...
                      " width="32" height="32" align="absmiddle"> M&Oacute;DULO: INSCRIPCI&Oacute;N DE ESTUDIANTES 
<?php 
if (isset($_GET["accion"])){
  $accion=strtoupper($_GET["accion"]);
	if ($accion!=="INSCRIBIR")
...
?></h1>
 ...
 <?php
					 if (!isset($_GET["accion"])){
						 ...
						 switch ($accion){
							 case "NUEVA":
							 	include_once("inscrip_est_nueva.php");
								break;
							 case "INSCRIBIR":
							 	include_once("inscrip_est_ins.php");
								break;	
							 case "BUSCAR":
							 	include_once("inscrip_est_bus.php");
								break;	
								
							 case "GENERA_INSCRIPCION":
							 	include_once("inscrip_est_guardar.php");
								guardar_datos();
								break;								
          ...
          <a id="nb" href="<?php echo "inscrip_est.php?id_func=".$_GET["id_func"]."&accion=buscar"?>"  title="Consultar datos">
                                <img src='../images/icons_menu/x32/search_x32.png' width='24px' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Consultar inscripci&oacute;n
          </a><br> 
            <?php 
            ...
            <?php          
						if ($array_permisos["crear"]==true) {?>
								<a id="an" href="<?php echo "inscrip_est.php?id_func=".$_GET["id_func"]."&accion=nueva" ?>"  title="Inscribir nuevo estudiante">
									<img src='../images/icons_menu/x32/00052.png' width='24' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Nueva inscripci&oacute;n&nbsp;               
                  </a><br>              
