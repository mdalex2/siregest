...
<form id="form_buscar" name="form_buscar" action="<?php echo "anno_esc_me.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="descrip_anno_esc" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="descrip_anno_esc") echo " selected ";?>>A&Ntilde;O ESCOLAR</OPTION>
<OPTION VALUE="cod_anno_esc" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="cod_anno_esc") echo " selected " ?>>C&Oacute;DIGO DE A&Ntilde;O ESCOLAR</OPTION>
</SELECT>
...
<div class="ctrlHolder">
  ...
  <label for="txt_buscar" id="lbl_text_busc"><?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba a&ntilde;o escolar a buscar"; ?></label><br>
...
<td>  
<button formaction="<?php echo "anno_esc_me.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select cod_anno_esc,descrip_anno_esc,visible,anno_esc_me.fecha_g,datos_per.nombres,datos_per.apellidos from (anno_esc_me INNER JOIN datos_per on anno_esc_me.guardado_por=datos_per.id_personal) ORDER BY fecha_fin DESC",true);} else {
		$consulta=ejecuta_sql("select cod_anno_esc,descrip_anno_esc,visible,anno_esc_me.fecha_g,datos_per.nombres,datos_per.apellidos from (anno_esc_me INNER JOIN datos_per on anno_esc_me.guardado_por=datos_per.id_personal) where  $campo_buscar LIKE '%$texto_buscar%' ORDER BY fecha_fin DESC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="anno_esc_me.php?id_func=".$_GET['id_func']."&accion=mostrar&id_mos=".$fila['cod_anno_esc'];
					echo '
...
<table id="tablasinbtn_desc" class="letra_16 mouse_hover" width="..."> 
        ...
        <?php
				...
				?>
          <tr>
            <td align="center"><?php echo $fila["cod_anno_esc"]?></td>
            <td><?php echo $fila["descrip_anno_esc"]?></td>
            <td align="center"><?php 
            ...
            <td align="center">
 							...
 							<a id="resize" href="anno_esc_me.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_mos={$fila['cod_anno_esc']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
