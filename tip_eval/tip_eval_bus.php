...
<form id="form_buscar" name="form_buscar" action="<?php echo "tip_eval.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="tipo_evaluacion" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="tipo_evaluacion") echo " selected ";?>>TIPO DE EVALUACI&Oacute;N</OPTION>
<OPTION VALUE="id_tip_eval" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="id_tip_eval") echo " selected " ?>>C&Oacute;DIGO</OPTION>
</SELECT>
...
<div class="ctrlHolder">
  ...
  <label for="txt_buscar" id="lbl_text_busc"><?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba tipo de evaluaci&oacute;n a buscar"; ?></label><br>
...
<td>  
<button formaction="<?php echo "tip_eval.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select id_tip_eval,tipo_evaluacion,tipo_evaluacion_abrev,visible,tip_eval_calif.fecha_g,datos_per.nombres,datos_per.apellidos from (tip_eval_calif INNER JOIN datos_per on tip_eval_calif.guardado_por=datos_per.id_personal) ORDER BY tipo_evaluacion ASC",true);} else {
		$consulta=ejecuta_sql("select id_tip_eval,tipo_evaluacion,tipo_evaluacion_abrev,visible,tip_eval_calif.fecha_g,datos_per.nombres,datos_per.apellidos from (tip_eval_calif INNER JOIN datos_per on tip_eval_calif.guardado_por=datos_per.id_personal) where  $campo_buscar LIKE '%$texto_buscar%' ORDER BY tipo_evaluacion ASC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="tip_eval.php?id_func=".$_GET['id_func']."&accion=mostrar&id_mos=".$fila['id_tip_eval'];
					echo '
...
<table id="tablasinbtn" class="letra_16 mouse_hover" width="..."> 
        <thead> 
          <tr>
            ...
            <th width="...">TIPO DE EVALUACI&Oacute;N&nbsp;</th>
        ...
        <?php
				...
				?>
          <tr>
            <td align="center"><?php echo $fila["id_tip_eval"]?></td>
            <td><?php echo $fila["tipo_evaluacion"]?></td>
            <td align="center"><?php echo $fila["tipo_evaluacion_abrev"]?></td>
            ...
            <?php if ($array_permisos["mostrar"]==true) {?>
            <td align="center">
 							<a id="resize" href="tip_eval.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_mos={$fila['id_tip_eval']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
