...
if ($consulta=ejecuta_sql($sql_obten_config_plan_estud,true)){
 ...
 if ($consul_asig){
	...
	while ($fila=mysql_fetch_array($consul_asig)){
		...
		//echo utf8_encode('<option value="'.$cod_asig_prog.'">'.$mat_prog_cor." - ".$des_mat_prog.'</option>');
		?>
    <input type="hidden" name="txt_cod_asig_pend[<?php echo $i;?>]" value="<?php echo $cod_asig_prog;?>" />
		<label>
      <input type="checkbox" name="chk_asig_pend[<?php echo $i;?>]" id="chk_asig_pend[<?php echo $i;?>]" /><?php echo ($des_mat_prog." - ".$mat_prog_cor);?></label>
    <br />
    <label>
<?php
	} // fin while
