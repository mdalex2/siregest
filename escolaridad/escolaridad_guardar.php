...
function guardar_datos(){
	if (empty($_POST)){ //si no se han recibido datos muestro error
			...
			$conexion=conectarse();
			$consulta_sql=mysql_query("insert into escolaridad  (id_escolaridad,escolaridad,escolaridad_abrev,observaciones,visible) values 
			('$cod_esc',
			'$escolaridad',
			'$escolaridad_abrev',
			'$obs',
			...
			if (!$consulta_sql){
				  $error=obtener_error(mysql_errno($conexion));
					...
					mostrar_box("err",false,"INFORMACIÓN","No se pudo guardar la escolaridad en la base de datos: <br>".$error);	
					echo mostrar_btn_imp_reg();
					}		else {
						$_SESSION["msg"]="Los datos de la escolaridad se guardaron correctamente";
