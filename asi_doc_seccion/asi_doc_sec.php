...
return false ;
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=<?php if (isset($_SESSION['juego_caracteres'])){ echo $_SESSION['juego_caracteres'];} else echo "utf-8";//esta variable se declara en modulo verificar login?>">
	<title><?php echo $_SESSION["app_nombre"]."-".$_SESSION["app_version"]; ?> - Asignaci&oacute;n de docentes a secciones</title>
  <!-- End of Meta -->
    <!-- Scripts Graybox -->
...
$(document).ready(function() {
		...
		$.post("carga_cmb_secc_bus.php",{ id:$(this).val() },function(data){$("#cmb_gra").html(data);})
		
	});	//------- FIN CARGA COMBO GRADO -----------------	

		//cargo lista de grados para el plantel seleccionado
		$("#btn_inc_prog").click(function(){
			//obtengo el codigo del plantel
			var cod_anno_esc = $('#txt_cod_anno_esc_ocu').val();
			var cod_secc = $('#txt_id_sec_ocu').val();
			var cod_prog = $('#cmb_prog option:selected').val();
			var id_profesor = $('#cmb_doc_prog option:selected').val();
			var observaciones = $('#txt_obs_prog').val();
			var id_docente_guia = $('#cmb_prof_guia option:selected').val();
			var id_func = "<?php echo $_GET["id_func"];?>";
		if ((id_profesor==null || id_profesor=="") || (cod_prog==null || cod_prog=="") || (id_docente_guia==null || id_docente_guia=="")){
			alert("Debe seleccionar el docente guía, programa y profesor que impartirá el programa");
		} else {
		$.post("carga_tabla_prog.php?cod_anno_esc="+cod_anno_esc+"&cod_secc="+cod_secc+"&cod_prog="+cod_prog+"&id_profesor="+id_profesor+"&observaciones="+observaciones+"&id_docente_guia="+id_docente_guia+"&id_func="+id_func,{ id:$(this).val() },function(data){$("#div_prog_ept").html(data);})
		$('#cmb_prog option:selected').removeAttr('selected');
		$('#cmb_doc_prog option:selected').removeAttr('selected');
		} // fin if faltan datos
	});//------- FIN CARGA COMBO GRADO	
 ...
 -->
					...
					<h2 align="left" class="titulo_form_blue">
                      ...
                      " width="32" height="32" align="absmiddle"> M&Oacute;DULO: ASIGNACI&Oacute;N DE DOCENTE A SECCI&Oacute;N
<?php 
if (isset($_GET["accion"])){
  $accion=strtoupper($_GET["accion"]);
 ...
 <?php
					 if (!isset($_GET["accion"])){
						 ...
						 switch ($accion){
						    ...
						    if (isset($_SESSION["msg"]) && $_SESSION["msg"]!=""){
									echo mostrar_box("inf",TRUE,"NOTIFICACI&Oacute;N",$_SESSION["msg"]);
									
									}
							 	include_once("asi_doc_sec_mos.php");
								break;
								case "BUSCAR":
							 	include_once("asi_doc_sec_bus.php");
								break;			
									
								case "ADMINISTRAR":
									include_once("asi_doc_sec_admin.php");
									break;	
								case "GUARDAR_ASIG":
									include_once("asi_doc_sec_gua_asi.php");
									break;																			
								case "IMPRIMIR_ASIGNACION":
									include_once("pla_est_fil_rep.php");
									break;	
								case "QUITAR_ASOCIACION":
									include_once("asi_doc_sec_eli_aso.php");
									quitar_asociacion();
									break;																											
						 } // fin switch
					 ?>          
          ...
          <?php
						...
						if ($array_permisos["crear"]==true) {?>
								<a id="an" href="<?php echo "asi_doc_sec.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>"  title="Permite administrar la asignaci&oacute;n de docentes para la secci&oacute;n seleccionada">
									<img src='../images/sistema/computer_go.png' width='24' height="24" style="padding:2px;" heigth='24px' align='absmiddle'></img>&nbsp;Asignaci&oacute;n de secciones&nbsp;               
                  </a><br>              
          ...
          <?php
						...
						if ($array_permisos["imprimir"]==true && !empty($_POST["cmb_year_esc"]) || (!empty($_GET["id_secc"]) && !empty($_GET["cod_anno_esc"]))) {?>
								<a id="rep" href="asi_doc_sec_rep.php?id_secc=<?php echo $id_seccion;?>&cod_anno_esc=<?php echo $anno_escolar;?>"  title="Ver reporte de asignaci&oacute;n de docentes" target="_blank">
									<img src='../images/icons_menu/x32/printer2_x32.png' width='24' height="24" style="padding:2px;" heigth='24px' align='absmiddle'></img>&nbsp;Generar reporte&nbsp;               
                  </a><br>              
          <?php } //cierro el si tiene permiso de crear ?>
        ...
        <?php 
          ...
          <a id="nb" href="<?php echo "asi_doc_sec.php?id_func=".$_GET["id_func"]."&accion=imprimir_asignacion"?>"  title="Ver asignación de materias al plan de estudio">
