...
{
var agree=confirm("¿Está seguro de eliminar este recaudo?");
if (agree)
return true ;
else
return false ;
...
{
var agree=confirm("¿ESTÁ SEGURO QUE DESEA ELIMINAR EL RECAUDO DE INSCRIPCIÓN \n\n Esto provocará que el mismo no sea requerido a la hora de inscribir alumnos. \n\n NOTA: SE RECOMIENDA NO ELIMINAR RECAUDOS AL MENOS QUE EL MISMO SE HALLA REGISTRADO POR ERROR");
if (agree){
	var agree=confirm("¿En realidad deseas eliminar el recaudo?\n\n Ésta acción no se podrá deshacer.");
	if (agree)
		return true ;}
else
return false ;
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=<?php if (isset($_SESSION['juego_caracteres'])){ echo $_SESSION['juego_caracteres'];} else echo "utf-8";//esta variable se declara en modulo verificar login?>">
	<title><?php echo $_SESSION["app_nombre"]."-".$_SESSION["app_version"]; ?> - Recaudos inscripciones</title>
  <!-- End of Meta -->
    <!-- Scripts Graybox -->
...
$(document).ready(function() {
 ...
 -->
					...
					<h1 align="left" class="titulo_form_blue">
                      ...
                      " width="32" height="32" align="absmiddle"> M&Oacute;DULO: RECAUDOS PARA INSCRIPCIONES
<?php 
if (isset($_GET["accion"])){
  $accion=strtoupper($_GET["accion"]);
...
?></h1><hr>
 ...
 <?php
					 if (!isset($_GET["accion"])){
						 ...
						 switch ($accion){
							 case "BUSCAR":
							 	include_once("tip_recaud_insc_bus.php");
								break;
							 case "NUEVO":
							 	include_once("tip_recaud_insc_crea.php");
								break;
							 case "MOSTRAR":
						    if (isset($_SESSION["msg"]) && $_SESSION["msg"]!=""){
							 	...
							 	include_once("tip_recaud_insc_mos.php");
								break;	
								case "GUARDAR":
							 	include_once("tip_recaud_insc_guardar.php");
								guardar_datos(); //llamo a la funcion que me guarda los datos en la BD
								break;			
								case "MODIFICAR":
							 	include_once("tip_recaud_insc_modif.php");
								modif_datos(); //llamo a la funcion que me guarda los datos en la BD
								break;
								case "ELIMINAR":
									include_once("tip_recaud_insc_elim.php");
									eliminar_datos();
									break;																			
          ...
          <a id="nb" href="<?php echo "tip_recaud_insc.php?id_func=".$_GET["id_func"]."&accion=buscar"?>"  title="Consultar datos">
            ...
            <?php          
						if ($array_permisos["crear"]==true) {?>
								<a id="an" href="<?php echo "tip_recaud_insc.php?id_func=".$_GET["id_func"]."&accion=nuevo" ?>"  title="Agregar nuevo">
          ...
          <?php } //cierro el si tiene permiso de crear ?>
            <?php //muestro botones de accion diponibles
						if ($array_permisos["imprimir"]==true) {?>
          <a id="rrc" href="<?php echo "reporte_recaudos.php?id_func=".$_GET["id_func"];?>"  title="Listado de recaudos por grado" class="fancybox.iframe resize">
                                <img src='../images/sistema/lupa_hoja.png' width='24' heigth='24px' align='absmiddle' style="padding:2px;"></img>&nbsp;Listado de recaudos
                              </a><br> 
            <?php 
						   } //cierro el si permite mostrar
						?>              
