...
<form id="form_buscar" name="form_buscar" action="<?php echo "dist_esc.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" method="post" enctype="multipart/form-data" class="uniForm">
<fieldset>
...
$("#txt_buscar").focus();'> 
<OPTION VALUE="num_dist" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="num_dist") echo " selected " ?>>N&deg; DE DISTRITO</OPTION>
<OPTION VALUE="dis_esc" <?php if (isset($_POST["cmb_tip_bus"]) && $_POST["cmb_tip_bus"]=="dis_esc") echo " selected " ?>>DISTRITO</OPTION>
</SELECT>
...
<div class="ctrlHolder">
  ...
  <label for="txt_buscar" id="lbl_text_busc"><?php if (isset($_POST["lbl_text_busc1"])) echo $_POST["lbl_text_busc1"]; else echo " Escriba n&uacute;mero de distrito a buscar"; ?></label><br>
...
<td>  
<button formaction="<?php echo "dist_esc.php?id_func=".$_GET["id_func"]."&accion=buscar" ?>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" type="submit">
...
<?php 
	if (isset($_POST["txt_buscar"]) && isset($_POST["cmb_tip_bus"]) && isset($_GET['id_func'])){
		...
		$consulta=ejecuta_sql("select id_dis_esc,num_dist,dis_esc,terr_estados.estado_ter,visible,dis_esc.fecha_g,datos_per.nombres,datos_per.apellidos from (dis_esc 
		INNER JOIN datos_per on dis_esc.guardado_por=datos_per.id_personal 
		INNER JOIN terr_estados on terr_estados.cod_estado_ter=dis_esc.cod_estado_ter

		) ORDER BY estado_ter,num_dist,dis_esc ASC",true);} else {
		$consulta=ejecuta_sql("select id_dis_esc,num_dist,dis_esc,terr_estados.estado_ter,visible,dis_esc.fecha_g,datos_per.nombres,datos_per.apellidos from (dis_esc 
		INNER JOIN datos_per on dis_esc.guardado_por=datos_per.id_personal 
		INNER JOIN terr_estados on terr_estados.cod_estado_ter=dis_esc.cod_estado_ter
		) where $campo_buscar LIKE '%$texto_buscar%' ORDER BY estado_ter,num_dist,dis_esc ASC",true);} 
		
		if ($consulta)
			{
				if (mysql_num_rows($consulta)==1) {
					$fila=mysql_fetch_array($consulta);
					$url="dist_esc.php?id_func=".$_GET['id_func']."&accion=mostrar&id_mos=".$fila['id_dis_esc'];
					echo '
...
<table id="tablasinbtn" class="letra_16 mouse_hover" width="..."> 
        <thead> 
          <tr>
            <th width="85px">N&deg; DIST.    </th>
            <th width="...">DISTRITO ESCOLAR</th>
             <th width="...">UBICACI&Oacute;N</th>
        ...
        <?php
				...
				?>
          <tr>
            <td><?php echo $fila["num_dist"]?></td>
            <td><?php echo $fila["dis_esc"]?></td>
            <td><?php echo $fila["estado_ter"]?></td>
            <td align="center"><?php 
            ...
            <td align="center">
 							...
 							<a id="resize" href="dist_esc.php?<?php echo "id_func=".$_GET['id_func']."&accion=mostrar&id_mos={$fila['id_dis_esc']}";?>" class=" ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" style="font-size:10px;padding:2px; color:#000000;" width="45%" height="90%" title="Mostrar" >&nbsp;
